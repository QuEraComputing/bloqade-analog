...
class Uniform(SpatialModulation):
    ...
    (See [`Waveform`][bloqade.analog.builder.waveform] for available waveform options)
    ...
    Examples:
        ...
        >>> reg = start.add_position([(0,0),(1,1),(2,2),(3,3)])
        >>> loc = reg.rydberg.detuning.uniform

        - Apply Linear waveform:

        >>> wv = bloqade.analog.ir.Linear(start=0,stop=1,duration=0.5)
        >>> reg = start.add_position([(0,0),(1,1),(2,2),(3,3)])
