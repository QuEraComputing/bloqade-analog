...
class Field(Builder):
    ...
    def uniform(self) -> "Uniform":
        ...
        - You can now do:
            ...
            - `...uniform.poly([coefficients], duration)` : to apply a
                polynomial waveform
            - `...uniform.apply(wf:bloqade.analog.ir.Waveform)`: to apply a
    ...
    def location(
        ...
        - You can now do:
            ...
            - `...location(labels, scales).apply(wf:bloqade.analog.ir.Waveform)`: to apply
    ...
    def scale(self, coeffs: Union[str, List[ScalarType]]) -> "Scale":
        ...
        - You can now do:
            ...
            - `...scale(coeffs).poly([coefficients], duration)` : to apply
                a polynomial waveform
            - `...scale(coeffs).apply(wf:bloqade.analog.ir.Waveform)`: to apply
                a pre-defined waveform
...
class Detuning(Field):
    ...
    Examples:
        ...
        >>> node = start.rydberg.detuning
        >>> type(node)
        <class 'bloqade.analog.builder.field.Detuning'>

        - To specify detuning of hyperfine coupling:

        >>> node = start.hyperfine.detuning
        >>> type(node)
        <class 'bloqade.analog.builder.field.Detuning'>

    Note:
        This node is a SpatialModulation node.
        See [`SpatialModulation`][bloqade.analog.builder.field.SpatialModulation]
...
class Rabi(Builder):
    ...
    Examples:
        ...
        >>> node = start.rydberg.rabi
        <class 'bloqade.analog.builder.field.Rabi'>

        - To specify rabi of hyperfine coupling:

        >>> node = start.hyperfine.rabi
        >>> type(node)
        <class 'bloqade.analog.builder.field.Rabi'>
...
class RabiAmplitude(Field):
    ...
    Examples:
        ...
        >>> node = start.rydberg.rabi.amplitude
        >>> type(node)
        <class 'bloqade.analog.builder.field.Amplitude'>

        - To specify rabi amplitude of hyperfine coupling:

        >>> node = start.hyperfine.rabi.amplitude
        >>> type(node)
        <class 'bloqade.analog.builder.field.Amplitude'>
...
class RabiPhase(Field):
    ...
    Examples:
        ...
        >>> node = start.rydberg.rabi.phase
        >>> type(node)
        <class 'bloqade.analog.builder.field.Phase'>

        - To specify rabi phase of hyperfine coupling:

        >>> node = start.hyperfine.rabi.phase
        >>> type(node)
        <class 'bloqade.analog.builder.field.Phase'>
