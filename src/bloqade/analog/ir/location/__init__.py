...
[`ListOfLocations`][bloqade.analog.ir.location.list.ListOfLocations].

- Next possible steps to build your program are:
- Specify which level coupling to address with:
    - `start.rydberg`: for [`Rydberg`][bloqade.analog.builder.coupling.Rydberg]
        Level coupling
    - `start.hyperfine`: for [`Hyperfine`][bloqade.analog.builder.coupling.Hyperfine]
        Level coupling
    ...
    - `start.add_position()`: to add atom(s) to current register. It will accept:
        - A single coordinate, represented as a tuple (e.g. `(5,6)`) with a value that
          can either be:
            ...
            - [`Scalar`][bloqade.analog.ir.scalar.Scalar] objects: `(2*cast("x"), 5+cast("y"))`
