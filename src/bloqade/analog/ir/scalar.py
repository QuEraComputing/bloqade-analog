...
class Scalar:
    ...
    def canonicalize(expr: "Scalar") -> "Scalar":
        from bloqade.analog.analog.compiler.rewrite.common.canonicalize import (
            Canonicalizer,
        )
...
def cast(py) -> "Scalar":
    ...
    to [`Scalar Literal`][bloqade.analog.ir.scalar.Literal].

    2. cast str (or list/tuple of Real numbers)
    to [`Scalar Variable`][bloqade.analog.ir.scalar.Variable].
...
def var(py: str) -> "Variable":
    ...
    to [`Variable`][bloqade.analog.ir.scalar.Variable].
